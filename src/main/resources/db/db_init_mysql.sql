...
-- -----------------------------------------------------
-- Table `celloperator`.`option`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `celloperator`.`option` ;

CREATE TABLE IF NOT EXISTS `celloperator`.`option` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL,
  `price_use` DECIMAL(4,2) NOT NULL,
  `archived` BIT(1) NOT NULL DEFAULT 0,
  `price_set` DECIMAL(4,2) NULL,
  ...
  CONSTRAINT `fk_option_option`
    ...
    REFERENCES `celloperator`.`option` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
INSERT INTO `celloperator`.`role` (`id`, `name`) VALUES (DEFAULT, 'customer');
